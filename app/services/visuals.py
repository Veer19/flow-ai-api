...
async def generate_visual_concepts(project: Project, data_sources: List[DataSource], stats: List[ProjectStats], relationships: List[Relationship]):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error generating chart ideas: {str(e)}")
...
async def get_visual_data(visual_concept: VisualConcept, data_sources: List[DataSource], relationships: List[Relationship]):
    ...
    try:
        ...
        result = execute_pandas_code(visual_python_code, dataframes, 'get_visual_data')
...
async def generate_visual_sample_data(visual_concept: VisualConcept, data_sources: List[DataSource], relationships: List[Relationship]):
    ...
    try:
        ...
        used_data_source_info = [ds.to_llm_dict() for ds in data_sources if str(ds.id) in visual_concept.data_sources_used]
        
        relationships_info = [r.to_llm_dict() for r in relationships]
        
        system_prompt = render_prompt("generate_visual_sample_data/system.jinja")
        user_prompt = render_prompt("generate_visual_sample_data/user.jinja", {
            "data_sources": ensure_json_serializable(used_data_source_info),
            "relationships": ensure_json_serializable(relationships_info),
...
async def generate_visual_python_code(data_sources: List[DataSource], relationships: List[Relationship], visual_concept: VisualConcept, visual_sample_data: VisualData):
    ...
    try:
        ...
        print(visual_concept.to_llm_dict())
        print(visual_sample_data)
        print("--------------------------------")
        
        system_prompt = render_prompt("generate_visual_python_code/system.jinja")
        ...
        logger.error(error_msg)
        raise HTTPException(status_code=500, detail=f"Error generating Python code: {str(e)}")
    ...
    try:
        ...
        tasks = [
            get_visual_data(                
                ...
                data_sources=[data_source for data_source in data_sources if data_source.id in concept.data_sources_used], 
                relationships=relationships
