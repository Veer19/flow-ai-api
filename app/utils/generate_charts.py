...
async def fetch_project_data_sources(project_id: str) -> Dict[str, Any]:
    try:
        ...
        data_sources_cursor = data_sources_collection.find(
    ...
    try:
        ...
        for ds in data_sources:
            source_info = {
                ...
                # Just a couple of rows for context
...
async def generate_chart(chart: Dict[str, Any]):
    ...
    try:
        ...
        if not data_sources:
            ...
            return {"error": error_msg, "success": False}
        logger.info(
            f"Generating ApexCharts configuration for chart: {chart.get('title')}")
    ...
    try:
        ...
        for ds in chart_data_sources:
            sample = {
                ...
                "columns": [col.get("name") for col in ds.get("columnMetadata", [])],
                "column_types": {col.get("name"): col.get("type") for col in ds.get("columnMetadata", [])},
                # Include more sample rows for better code generation
                "sample_data": ds.get("sampleData", [])[:5],
                "stats": ds.get("stats", {})
            }
...
async def generate_chart_real_data(project_id: str, chart_info: Dict[str, Any], sample_series: List[Dict[str, Any]], sample_options: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    if not chart_data_sources:
        logger.error(
            f"Could not find specified data sources for chart: {chart_info.get('title')}")
        # Return sample data if we can't find the data sources
        return sample_series, sample_options
    ...
    for ds in chart_data_sources:
        try:
            ...
            if not blob_path:
                logger.warning(
                    f"No blob path found for data source: {ds.get('filename')}")
                continue
            ...
            if not blob_content:
                logger.warning(
                    f"Could not download blob for data source: {ds.get('filename')}")
                continue
            ...
            try:
                ...
                logger.info(
                    f"Successfully loaded data for {ds.get('filename')} with {len(df)} rows")
                ...
                try:
                    ...
                    dataframes[ds.get("filename")] = df
                    logger.info(
                        f"Successfully loaded data for {ds.get('filename')} with {len(df)} rows using latin1 encoding")
                except Exception as encoding_error:
                    try:
                        # If Latin-1 fails, try with error handling
                        df = pd.read_csv(io.BytesIO(
                            blob_content), encoding='utf-8', errors='replace')
                        dataframes[ds.get("filename")] = df
                        logger.info(
                            f"Successfully loaded data for {ds.get('filename')} with {len(df)} rows using error replacement")
                    except Exception as replace_error:
                        logger.error(
                            f"Failed to read {ds.get('filename')} with multiple encodings: {str(replace_error)}")
                        continue

        except Exception as e:
            logger.error(
                f"Error loading data for {ds.get('filename')}: {str(e)}")
            continue

    if not dataframes:
        logger.error("Could not load any data from blob storage")
        # Return sample data if we couldn't load any real data
        return sample_series, sample_options

    # Filter data_info based on the available dataframes
    data_sources = [ds for ds in data_sources if ds.get(
        "filename") in dataframes.keys()]
